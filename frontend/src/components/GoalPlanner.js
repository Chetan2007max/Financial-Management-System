...
const steps = ['Goal Details', 'Financial Info'];

function GoalPlanner() {
  const [activeStep, setActiveStep] = useState(0);
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({ goal_amount: '', months: '', income: '', expenses: '', age: '' });
  ...
  const validateStep = () => {
    const newErrors = {};
    if (activeStep === 0) {
      if (!formData.goal_amount || isNaN(formData.goal_amount) || formData.goal_amount <= 0) newErrors.goal_amount = 'Enter valid goal amount';
      if (!formData.months || isNaN(formData.months) || formData.months <= 0) newErrors.months = 'Enter valid time in months';
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://127.0.0.1:8000/recommend/goal', formData);
      navigate('/results', { state: { type: 'goal', data: response.data.recommendation } });
    } catch (error) {
      setLoading(false);
        ...
        return (
          <Box sx={{ mt: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              ...
              <Typography variant="h6" sx={{ ml: 1 }}>Goal Details</Typography>
            </Box>
            <TextField
              fullWidth
              label="Goal Amount"
        ...
        return (
          <Box sx={{ mt: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              ...
              <Typography variant="h6" sx={{ ml: 1 }}>Financial Info</Typography>
            </Box>
            <TextField
              fullWidth
